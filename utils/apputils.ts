...
export const deviceNotification = async (text: string) => {
...
export const getTaskGroups = async (userid: string) => {
  const taskGroupsRef = collection(db, `users/${userid}/taskGroups`);
  // const taskGroupsRef = collectionGroup(
  // 	db,
  // 	`users/${userid}/taskGroups`
  // )
  const q = query(taskGroupsRef, orderBy('timestamp', 'desc'));
  ...
  for (const doc of res) {
    ...
    doc.activeTasks = taskArray.filter(el => el.data.checked === false).length;
    doc.archivedTasks = taskArray.filter(
      el => el.data.archived === true,
    ).length;

    doc.urgentTasks = taskArray.filter(
      el => el.data.urgency === 'urgent' && el.data.checked === false,
    ).length;

    doc.warningTasks = taskArray.filter(
      el => el.data.urgency === 'warning' && el.data.checked === false,
    ).length;
  }
...
export const getTaskGroups_old = async (userid: string) => {
  const taskGroupsRef = collection(db, `users/${userid}/taskGroups`);
  const q = query(taskGroupsRef, orderBy('timestamp', 'desc'));
  const querySnapshot = await getDocs(q);
...
export const extractCapitals = (text: string) => {
  if (text.trim().length < 5)
    return `${text.trim()[0].toUpperCase()}${text.trim()[1].toUpperCase()}`;
  if (text.split(' ').length > 2)
    return `${text.split(' ')[0][0].toUpperCase()}${text
      .split(' ')[1][0]
      .toUpperCase()}${text.split(' ')[2][0].toUpperCase()}`;
  if (text.split(' ').length > 1)
    return `${text.split(' ')[0][0].toUpperCase()}${text
      .split(' ')[1][0]
      .toUpperCase()}`;

  return `${text.trim()[0].toUpperCase()}${text.trim()[1].toUpperCase()}${text
    .trim()[2]
    .toUpperCase()}`;
};
...
export const refreshTaskData = async ({
  ...
  getTaskGroups(userid).then(async res => {
    await getSharedGroups(userid).then(async res => {
      ...
      if (res.length > 0) {
        const populatedData = await populateExternalTaskGroups(res);
      ...
      if (res.length > 0 && externalTaskGroupIndex > -1) {
        ...
        await getTasks(
          res[externalTaskGroupIndex].fromUser,
          `${res[externalTaskGroupIndex].taskGroup}`,
          true,
        ).then(res => {
    ...
    if (res.length > 0 && taskGroupIndex > -1) {
      getTasks(userid, res[taskGroupIndex].id).then(res => {
        setTasks(res);
